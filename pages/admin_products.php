...
<div class="max-w-7xl mx-auto px-4 py-8 mb-14 afiu">
    <div class="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
        <h1 class="text-2xl font-black text-pri-900 font-amiri"><i class="fas fa-box-open text-gld-500 ml-2"></i>إدارة المنتجات والملفات</h1>
        <div class="flex gap-2">
            ...
            <a href="index.php?page=admin_product_form" class="cf-btn cf-btn-pri cf-btn-sm"><i class="fas fa-plus"></i> منتج / ملف جديد</a>
        </div>
    </div>

    <?php if(isset($_GET['deleted'])): ?>
        <div class="bg-green-50 border-r-4 border-green-500 p-4 rounded-xl text-green-700 font-bold mb-6">تم حذف العنصر بنجاح.</div>
    <?php endif; ?>
                    ...
                    <?php else: ?>
                        <?php foreach($products as $p): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if($p['is_digital'] == 1): ?>
                                        <span class="badge bg-blue-50 text-blue-600 border border-blue-200"><i class="fas fa-file-download ml-1"></i> ملف رقمي</span>
                                    <?php else: ?>
                                        <span class="font-bold <?= $p['stock_quantity'] < 5 ? 'text-red-500' : 'text-green-600' ?>"><?= $p['stock_quantity'] ?> وحدة</span>
                                    <?php endif; ?>
                                </td>
                                <td>
